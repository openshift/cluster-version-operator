...
func (c *nodeInformerController) sync(ctx context.Context, syncCtx factory.SyncContext) error {
	...
	case nodeKindName:
		...
		mcp := c.whichMCP(node)
		if mcp == nil {
			// No insight is generated for the node which does not belong to any MCP.
			// At any time, any node belongs to one and only one MCP.
			// Otherwise, it indicates a configuration error on the cluster.
			klog.Errorf("Failed to determine which machine config pool the node %s belongs to in %d pools from the cache",
				node.Name, len(c.machineConfigPoolCache))
			return nil
		}
...
func (c *nodeInformerController) reconcileAllNodes(queue workqueue.TypedRateLimitingInterface[any]) error {
	...
	for _, node := range nodes {
		queue.Add(nodeNameToQueueKey(node.Name))
	}
	return nil
}
...
func nodeInformerControllerQueueKeys(object runtime.Object) []string {
	...
	case *corev1.Node:
		return []string{nodeNameToQueueKey(o.Name)}
	case *machineconfigv1.MachineConfig:
		return []string{fmt.Sprintf("%s/%s", machineConfigKindName, o.Name)}
	case *machineconfigv1.MachineConfigPool:
		return []string{fmt.Sprintf("%s/%s", machineConfigPoolKindName, o.Name)}
	default:
		panic(fmt.Sprintf("USC :: Unknown object type: %T", object))
	}
}

func nodeNameToQueueKey(nodeName string) string {
	return fmt.Sprintf("%s/%s", nodeKindName, nodeName)
}
