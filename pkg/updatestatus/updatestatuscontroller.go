...
//
// The communication between informers (insight producers) and this controller is performed via a channel.
// The newUpdateStatusController constructor calls the startCollectingInsights function that creates the channel spawns
// a goroutine that consumes the channel and updates the internal ConfigMap state with the received insights. The constructor
// then creates a function that other controllers can use to send insights to this controller, using the informerMsg
// structure as a data transfer object.
...
func newUpdateStatusController(
	...
	// Spawn the insight collecting goroutine
	sendInsight := c.startCollectingInsights(ctx)

	return controller, sendInsight
}

// startCollectingInsights starts a goroutine that listens for insights from informers and updates internal status
// API-mimicking ConfigMap.
func (c *updateStatusController) startCollectingInsights(ctx context.Context) sendInsightFn {
	...
	go func() {
		...
		for {
			...
			case insight := <-fromInformers:
				...
				_ = c.commitStatusApiAsConfigMap(ctx)
			case <-ctx.Done():
