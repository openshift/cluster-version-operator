...
func assessClusterVersion(cv *configv1.ClusterVersion, now metav1.Time) *ClusterVersionStatusInsight {
	...
	var assessment ControlPlaneUpdateAssessment
	...
	case metav1.ConditionUnknown:
		assessment = ControlPlaneUpdateAssessmentUnknown
	default:
		assessment = ControlPlaneUpdateAssessmentUnknown
	}
	...
	return &ClusterVersionStatusInsight{
		Resource: ResourceRef{
			Kind:     "ClusterVersion",
			APIGroup: configv1.GroupName,
			Name:     cv.Name,
		},
		...
		EstimatedCompletedAt: metav1.NewTime(estimateCompletion(startedAt.Time)),
...
func setCannotDetermineUpdating(cond *metav1.Condition, message string) {
	...
	cond.Reason = string(ClusterVersionStatusInsightUpdatingReasonCannotDetermineUpdating)
...
func cvProgressingToUpdating(cvProgressing configv1.ClusterOperatorStatusCondition) (metav1.ConditionStatus, string, string) {
	...
	switch status {
	case metav1.ConditionTrue:
		reason = string(ClusterVersionStatusInsightUpdatingReasonProgressing)
	case metav1.ConditionFalse:
		reason = string(ClusterVersionStatusInsightUpdatingReasonNotProgressing)
	case metav1.ConditionUnknown:
		reason = string(ClusterVersionStatusInsightUpdatingReasonCannotDetermineUpdating)
	default:
		reason = string(ClusterVersionStatusInsightUpdatingReasonCannotDetermineUpdating)
	}
...
func versionsFromHistory(history []configv1.UpdateHistory) ControlPlaneUpdateVersions {
	...
	if len(history) == 1 {
		versions.Target.Metadata = []VersionMetadata{
	...
	if len(history) > 1 {
		...
		if history[1].State == configv1.PartialUpdate {
			versions.Previous.Metadata = []VersionMetadata{
