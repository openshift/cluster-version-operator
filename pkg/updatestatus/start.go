...
func Run(ctx context.Context, cc *controllercmd.ControllerContext) error {
	...
	updateInformers := updateinformers.NewSharedInformerFactory(updateClient, 10*time.Minute)
	machineConfigInformers := machineconfiginformers.NewSharedInformerFactory(machineConfigClient, 10*time.Minute)

	updateStatusController, sendInsight := newUpdateStatusController(updateClient.UpdateV1alpha1(), updateInformers, cc.EventRecorder)
