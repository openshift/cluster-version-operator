...
func Test_sync(t *testing.T) {
	...
	cvRef := ResourceRef{Kind: "ClusterVersion", APIGroup: "config.openshift.io", Name: "version"}
	...
	testCases := []struct {
		...
		expectedMsgs map[string]UpdateInsight
	}{
		{
			...
			expectedMsgs: map[string]UpdateInsight{
				"usc-cv-version": {
					Type:       UpdateInsightTypeClusterVersionStatusInsight,
					UID:        "usc-cv-version",
					AcquiredAt: now,
					ClusterVersionStatusInsight: &ClusterVersionStatusInsight{
						Resource:   cvRef,
						Assessment: ControlPlaneUpdateAssessmentProgressing,
						Versions: ControlPlaneUpdateVersions{
							Target: UpdateEdgeVersion{
								Version: "4.18.0",
								Metadata: []VersionMetadata{
									{
										Key:  InstallationMetadataKey,
										Type: VersionMetadataTypeBool,
										Bool: true,
									},
								},
							},
						...
						Conditions: []metav1.Condition{
							{
								Type:               string(ClusterVersionStatusInsightConditionTypeUpdating),
								Status:             metav1.ConditionTrue,
								Reason:             "ClusterVersionProgressing",
								Message:            "ClusterVersion has Progressing=True(Reason=ProgressingTrue) | Message='Cluster is progressing'",
		...
		},
		{
			...
			expectedMsgs: map[string]UpdateInsight{
				"usc-cv-version": {
					...
					ClusterVersionStatusInsight: &ClusterVersionStatusInsight{
						Resource:   cvRef,
						Assessment: ControlPlaneUpdateAssessmentCompleted,
						Versions: ControlPlaneUpdateVersions{
							Target: UpdateEdgeVersion{
								Version: "4.18.0",
								Metadata: []VersionMetadata{
						...
						Conditions: []metav1.Condition{
							{
								...
								Message:            "ClusterVersion has Progressing=False(Reason=ProgressingFalse) | Message='Cluster is on version 4.X.0'",
								LastTransitionTime: now,
							},
						},
					},
			...
			expectedMsgs: map[string]UpdateInsight{
				"usc-cv-version": {
					...
					ClusterVersionStatusInsight: &ClusterVersionStatusInsight{
						...
						Assessment: ControlPlaneUpdateAssessmentProgressing,
						Versions: ControlPlaneUpdateVersions{
							Target:   UpdateEdgeVersion{Version: "4.19.0"},
							Previous: UpdateEdgeVersion{Version: "4.18.0"},
						},
						Completion:           0,
	...
	testCases := []struct {
	...
	}{
		{
			...
			expected: &ClusterVersionStatusInsight{
				...
				EstimatedCompletedAt: metav1.NewTime(now.Add(30 * time.Minute)),
				Conditions: []metav1.Condition{
					{
						Type:    string(ClusterVersionStatusInsightConditionTypeUpdating),
						Status:  metav1.ConditionTrue,
						Reason:  "ClusterVersionProgressing",
						Message: "ClusterVersion has Progressing=True(Reason=WhateverCVOHasHereWhenInstalling) | Message='Whatever CVO has as a message while installing'",
					},
				},
			},
		},
				...
				Conditions: []metav1.Condition{
					{
						...
						Status:  metav1.ConditionFalse,
						Reason:  "ClusterVersionNotProgressing",
						Message: "ClusterVersion has Progressing=False(Reason=) | Message='Cluster version is 4.18.0'",
					},
				},
			},
		},
		{
			...
			expected: &ClusterVersionStatusInsight{
				...
				EstimatedCompletedAt: metav1.NewTime(now.Add(40 * time.Minute)),
				Conditions: []metav1.Condition{
					{
						Type:    string(ClusterVersionStatusInsightConditionTypeUpdating),
						Status:  metav1.ConditionTrue,
						Reason:  "ClusterVersionProgressing",
						Message: "ClusterVersion has Progressing=True(Reason=WhateverCVOHasHereWhenUpdating) | Message='Whatever CVO has as a message while updating'",
					},
				},
			},
		},
		...
		},
		{
			...
			expected: &ClusterVersionStatusInsight{
				...
				Assessment: ControlPlaneUpdateAssessmentProgressing,
				Versions: ControlPlaneUpdateVersions{
					Target: UpdateEdgeVersion{Version: "4.19.1"},
					Previous: UpdateEdgeVersion{
						Version: "4.19.0",
						Metadata: []VersionMetadata{
							{
								Key:  PartialMetadataKey,
								Type: VersionMetadataTypeBool,
								Bool: true,
							},
						},
				...
				Conditions: []metav1.Condition{
					{
						...
						Status:  metav1.ConditionFalse,
						Reason:  "ClusterVersionNotProgressing",
						Message: "ClusterVersion has Progressing=False(Reason=) | Message='Cluster version is 4.19.1'",
					},
				},
			},
		},
...
func Test_isControlPlaneUpdating(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.expectedCondition.Type = string(ClusterVersionStatusInsightConditionTypeUpdating)
		...
		},
		{
			...
			expected: ControlPlaneUpdateVersions{
				Target: UpdateEdgeVersion{
					Version: "4.18.0",
					Metadata: []VersionMetadata{
						{Key: InstallationMetadataKey, Type: VersionMetadataTypeBool, Bool: true},
					},
				},
			},
		},
		...
		},
		{
			...
			expected: ControlPlaneUpdateVersions{
				...
				Previous: UpdateEdgeVersion{
					Version: "4.19.0",
					Metadata: []VersionMetadata{
						{Key: PartialMetadataKey, Type: VersionMetadataTypeBool, Bool: true},
					},
				},
			},
		},
