...
func (m *operatorMetrics) Collect(ch chan<- prometheus.Metric) {
	...
	if cv, err := m.optr.cvLister.Get(m.optr.name); err == nil {
		...
		failing := resourcemerge.IsOperatorStatusConditionTrue(cv.Status.Conditions, cvv1.OperatorFailing)
	...
	for _, op := range operators {
		...
		failing := resourcemerge.IsOperatorStatusConditionTrue(op.Status.Conditions, cvv1.OperatorFailing)
		available := resourcemerge.IsOperatorStatusConditionTrue(op.Status.Conditions, cvv1.OperatorAvailable)
