...
func (w *SyncWorker) syncPayload(ctx context.Context, work *SyncWork) ([]configv1.ClusterVersionCapability, error) {
	...
	implicitlyEnabledCaps := capability.SortedList(work.Capabilities.ImplicitlyEnabled)
...
func (w *SyncWorker) apply(ctx context.Context, work *SyncWork, maxWorkers int, previousStatus *SyncWorkerStatus) error {
	...
	errs := payload.RunGraph(ctx, graph, maxWorkers, func(ctx context.Context, tasks []*payload.Task) error {
		...
		for _, task := range tasks {
			...
			// Note: We do not pass all filters here as some filtering cannot change between loading the payload and applying it.
			// For example, the major version, cluster profile and featureset should not have changed, filtering again here would be redundant.
