...
//
// g := newGraph(tasks)
// g.Split(OnJobs) -> jobs serialize the graph
// g.Parallelize(ByNumberAndComponent) -> if a node has 0000_99_A_* and 0000_99_B_*, parallelize

// task has type, name, grouping 0000_XX_yy-yy-yy_ORDERED
// all groups with common XX and different yy-yy-yy may be executed in parallel (0000_99_cluster-monitoring-operator and 0000_99_cluster-samples-operator)
// within a group:
//   - all executable or unrecognized types form synchronization barriers (ClusterOperator, Deployment, Pod are all sync barriers)
//   - all other types may be greedily created (Namespace, RoleBinding, Service, Route)
// all jobs are global synchronization barriers
//
...
// the same 0000_NN but different NAME can be run in parallel.
func ByNumberAndComponent(tasks []*Task) [][]*TaskNode {
	...
	for i := 0; i < count; {
		...
		for ; j < count; j++ {
			next := matches[j]
			if match == nil || next == nil || match[1] != next[1] {
				break
			}
			if match[2] != next[2] {
...
func RunGraph(ctx context.Context, graph *TaskGraph, maxParallelism int, fn func(ctx context.Context, tasks []*Task) error) error {
	...
	go func() {
		...
		// waiting for completion (1), or have completed (2)
		const (
		...
		for i := 0; i < len(visited); i++ {
			if visited[i] == 0 && canVisit(graph.Nodes[i]) {
	...
	err := <-errCh
	glog.V(4).Infof("Result of work: %v", err)
	wg.Wait()
	glog.V(4).Infof("All workers shut down, exiting")
	return err
}
