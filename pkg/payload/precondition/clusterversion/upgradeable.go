...
func (pf *Upgradeable) Run(ctx context.Context, releaseContext precondition.ReleaseContext) error {
	...
	klog.V(2).Infof("The current version is %s parsed from %s and the target version is %s parsed from %s", currentVersion.String(), cv.Status.Desired.Version, targetVersion.String(), releaseContext.DesiredVersion)
	if targetVersion.Major == currentVersion.Major && targetVersion.Minor == currentVersion.Minor {
		...
		if condition := ClusterVersionOverridesCondition(cv); condition != nil {
			klog.V(2).Infof("Update from to %s is blocked by %s: %s", targetVersion.String(), condition.Reason, condition.Message)
			return &precondition.Error{
