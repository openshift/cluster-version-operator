...
func Test_New(t *testing.T) {
	...
	t.Run("quick change notification on Upgradeable change to False", func(t *testing.T) {
		...
		select {
		case <-changeChannel:
			t.Fatal("updating the local pointer triggered a change notification, when there should not have been change triggered until the fake client update")
		...
		t.Run("expected Risks output", func(t *testing.T) {
			...
			if _, _, err := risk.Merge(nil, nil, expectedRisks, nil); err != nil {
				t.Errorf("unexpected error: %v", err)
			}
		})
	})
