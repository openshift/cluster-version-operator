...
func TestIntegrationCVO_initializeAndUpgrade(t *testing.T) {
	...
	cvConfigInformerFactory, configInformerFactory := options.prepareConfigInformerFactories(cb)
	featureset, gates, err := options.processInitialFeatureGate(context.Background(), configInformerFactory)
	if err != nil {
		t.Fatal(err)
	}
	controllers, err := options.NewControllerContext(cb, featureset, gates, cvConfigInformerFactory, configInformerFactory)
	if err != nil {
		t.Fatal(err)
	}
