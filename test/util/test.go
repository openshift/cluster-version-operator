...
func AnnotateTestSuite() {
	...
	waitErr := wait.PollUntilContextTimeout(context.Background(), 3*time.Second, 30*time.Second, true, func(context.Context) (bool, error) {
...
func PreDetermineExternalOIDCCluster() (bool, error) {
	...
	var errAuth error
	err = wait.PollUntilContextTimeout(context.Background(), 3*time.Second, 9*time.Second, true, func(context.Context) (bool, error) {
