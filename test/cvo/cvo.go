...
var _ = g.Describe(`[Jira:"Cluster Version Operator"] cluster-version-operator`, func() {
	...
	g.It("cvo drops invalid conditional edges", ote.Informing(), g.Label("46422", "Low", "ConnectedOnly", "Serial"), func() {
		...
		deployment1, err := util.RunUpdateService(ctx, kubeClient, ns.Name, buf.String(), label)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		defer func() { _ = util.DeleteDeployment(ctx, kubeClient, ns.Name, deployment1.Name) }()
		...
		_, err = util.CreateNetworkPolicy(ctx, kubeClient, ns.Name, policyName, label)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		deployment2, err := util.RunUpdateService(ctx, kubeClient, ns.Name, buf.String(), label)
		o.Expect(err).NotTo(o.HaveOccurred())
		defer func() { _ = util.DeleteDeployment(ctx, kubeClient, ns.Name, deployment2.Name) }()
